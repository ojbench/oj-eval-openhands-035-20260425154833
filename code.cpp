...
static void print_line(const MyString& s) {
    std::cout << s.c_str() << '\n';
}
...
int main() {
    ...
    MyString s;
    std::string cmd;
    // Try support both styles: optional leading Q, or until EOF
    long long Q = -1;
    if (std::cin.peek() != EOF) {
        std::streampos pos = std::cin.tellg();
        long long t; if (std::cin >> t) {
            if (std::cin.peek()=='\n' || std::cin.peek()==' ') {
                Q = t;
            } else {
                std::cin.clear();
    ...
    auto run_one = [&](const std::string& c)->bool{
        if (c=="assign" || c=="setstr" || c=="1") {
            std::string x; if(!(std::cin>>x)) return false; s = MyString(x.c_str());
        } else if (c=="append" || c=="2") {
            std::string x; if(!(std::cin>>x)) return false; s.append(x.c_str());
        } else if (c=="reserve" || c=="3") {
            size_t n; if(!(std::cin>>n)) return false; s.reserve(n);
        } else if (c=="resize" || c=="4") {
            size_t n; if(!(std::cin>>n)) return false; s.resize(n);
        } else if (c=="size" || c=="5") {
            std::cout << s.size() << '\n';
        } else if (c=="capacity" || c=="cap" || c=="6") {
            std::cout << s.capacity() << '\n';
        } else if (c=="get" || c=="at" || c=="7") {
            size_t i; if(!(std::cin>>i)) return false; try{ std::cout << s.at(i) << '\n'; }catch(const std::out_of_range&){ std::cout << "\n"; }
        } else if (c=="set" || c=="8") {
            size_t i; char ch; if(!(std::cin>>i>>ch)) return false; try{ s[i]=ch; }catch(const std::out_of_range&){}
        } else if (c=="plus" || c=="+" || c=="9") {
            std::string x; if(!(std::cin>>x)) return false; MyString rhs(x.c_str()); s = s + rhs;
        } else if (c=="cstr" || c=="print" || c=="10") {
            print_line(s);
        } else if (c=="iter_print" || c=="11") {
            for (auto it = s.begin(); it != s.end(); ++it) std::cout << *it; std::cout << '\n';
        } else if (c=="quit" || c=="exit") {
            return false;
        } else {
            // Unknown: try treat as a raw string to assign
            s = MyString(c.c_str());
        }
        return true;
    };
    ...
    if (Q >= 0) {
        for (long long i=0;i<Q;i++) { if(!(std::cin>>cmd)) break; if(!run_one(cmd)) break; }
    } else {
        while (std::cin>>cmd) { if(!run_one(cmd)) break; }
    }
    return 0;
}
